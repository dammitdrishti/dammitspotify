...
const App = () => {
  ...
  const [vibeBuckets, setVibeBuckets] = useState<
  ...
  const handleYourEras = async () => {
    ...
    try {
      const [long, medium, short] = await Promise.all([
        fetchTopTracks(token, 15, "long_term"),
        fetchTopTracks(token, 15, "medium_term"),
        fetchTopTracks(token, 15, "short_term"),
      ]);

      const p1 = long.map((t: any) => ({ ...t, phase: 1 }));
      const p2 = medium.map((t: any) => ({ ...t, phase: 2 }));
      const p3 = short.map((t: any) => ({ ...t, phase: 3 }));

      const uniqueMap = new Map();
      [...p1, ...p2, ...p3].forEach((t) => uniqueMap.set(t.id, t));
      const sorted = Array.from(uniqueMap.values()).sort(
        (a: any, b: any) => a.phase - b.phase,
      );

      startSelection(sorted, "Your Eras", true);
    } catch (err: any) {
      handleError(err);
    } finally {
      setLoading(false);
  ...
  const handleGatekeeper = async () => {
    ...
    setError(null);
    try {
      const allTracks = await fetchTopTracks(token, 50, "medium_term");
  ...
  const handleSonicAura = async () => {
    ...
    try {
      ...
      const featuresMap = new Map(
        features.filter((f) => f).map((f) => [f.id, f]),
      );

      const getFeat = (track: any) =>
      ...
      pickWinner(VibeMode.TIME_TRAVELER, (a, b) =>
      ...
      pickWinner(
        VibeMode.BEAST,
        (a, b) => getFeat(b).energy - getFeat(a).energy,
      );
      pickWinner(
        VibeMode.LATE_NIGHT,
        (a, b) => getFeat(b).danceability - getFeat(a).danceability,
      );
      pickWinner(
        VibeMode.MAIN_CHAR,
        (a, b) => getFeat(b).valence - getFeat(a).valence,
      );
...
const LoginView = ({ error, loading, onLogin }: any) => (
  <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-950 relative">
    ...
    <button
      onClick={onLogin}
      disabled={loading}
      className="flex items-center gap-2 bg-green-500 text-black font-bold py-3 px-8 rounded-full hover:scale-105 transition-transform"
    >
      {loading ? (
    ...
    {error && (
      <p className="text-red-500 mt-4 bg-red-900/20 px-4 py-2 rounded">
        {error}
      </p>
    )}
  </div>
...
const Header = ({ logout }: any) => (
  <header className="flex justify-between items-center mb-8">
    <div className="flex items-center gap-2">
      <Music className="text-green-500" /> <b>Spotify Insights</b>
    </div>
...
const LoadingOverlay = () => (
  <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 flex-col gap-4">
    <Loader2 className="animate-spin text-green-500 w-12 h-12" />
    <p className="text-green-500 animate-pulse">Analyzing Library...</p>
  </div>
);
const ErrorBanner = ({ message }: any) => (
  <div className="bg-red-900/50 text-red-200 p-4 rounded mb-6 text-center">
    {message}
  </div>
);

// --- DASHBOARD (Using Local Images) ---
const Dashboard = ({ onYourEras, onGatekeeper, onSonicAura }: any) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
    {/* MAKE SURE TO UPLOAD spot1.png, spot2.png, spot3.png TO YOUR PUBLIC FOLDER */}
    <ImageCard
      onClick={onYourEras}
      title="Your Eras"
      desc="Trace the chronology of your year."
      imageSrc="/spot1.png"
    />
    <ImageCard
      onClick={onGatekeeper}
      title="Gatekeeper Score"
      desc="Flex your underrated favorites."
      imageSrc="/spot2.png"
    />
    <ImageCard
      onClick={onSonicAura}
      title="Sonic Aura"
      desc="The ultimate vibe check."
      imageSrc="/spot3.png"
    />
  </div>
);

const ImageCard = ({ onClick, imageSrc, title, desc }: any) => (
  <div
    onClick={onClick}
    className="relative cursor-pointer rounded-3xl overflow-hidden hover:scale-105 transition-transform shadow-lg shadow-black/50 h-96 group"
  >
    <img
      src={imageSrc}
      alt={title}
  ...
  const vibes = [
    {
      ...
      color: "red",
      desc: "Highest Energy (Motivation)",
    },
    {
      id: VibeMode.MAIN_CHAR,
      icon: <Smile className="w-5 h-5" />,
      color: "pink",
      desc: "Highest Valence (Happiness)",
    },
    {
      id: VibeMode.VILLAIN,
      icon: <Zap className="w-5 h-5" />,
      color: "orange",
      desc: "High Energy + Low Valence (Angry)",
    },
    {
      id: VibeMode.LATE_NIGHT,
      icon: <Moon className="w-5 h-5" />,
      color: "indigo",
      desc: "Highest Danceability",
    },
    {
      id: VibeMode.DOOMSCROLLING,
      icon: <Frown className="w-5 h-5" />,
      color: "gray",
      desc: "Heard for hours on loop",
    },
    {
      id: VibeMode.TIME_TRAVELER,
      icon: <Clock className="w-5 h-5" />,
      color: "cyan",
      desc: "Oldest Release Date",
    },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {vibes.map((v) => {
          ...
          return (
            <div
              key={v.id}
              onClick={() => topSong && onSelect(v.id)}
              className={`relative overflow-hidden bg-zinc-900 border border-zinc-800 rounded-2xl p-6 transition-all group h-64 flex flex-col justify-between ${topSong ? "cursor-pointer hover:border-white/20" : "opacity-50 cursor-not-allowed"}`}
            >
              {topSong && (
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div
                  className={`p-3 rounded-xl bg-zinc-950/50 backdrop-blur text-${v.color}-400`}
                >
                  {v.icon}
                </div>
              </div>
              <div className="relative z-10">
                <h3
                  className={`text-2xl font-bold mb-1 group-hover:text-${v.color}-400 transition-colors`}
                >
                  {v.id}
                </h3>
  ...
  const renderTrackList = (tracks: any[]) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {tracks.map((track: any) => {
        ...
        const style = isUnderground
          ? getUniquenessColor(track.popularity)
          : null;

        return (
          <div
            key={track.id}
            onClick={() => !isDisabled && onToggle(track.id)}
            className={`flex items-center gap-4 p-3 rounded-xl border transition-all cursor-pointer ${isSelected ? "bg-green-900/20 border-green-500" : isDisabled ? "opacity-40 border-transparent" : "bg-black/20 border-white/5 hover:bg-black/40"}`}
          >
            <div
              className={`w-6 h-6 rounded-full border flex items-center justify-center shrink-0 ${isSelected ? "bg-green-500 border-green-500" : "border-zinc-600"}`}
            >
            ...
            <div className="overflow-hidden flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {isUnderground && (
                  ...
                  >
                    {100 - track.popularity}% ({style?.label})
                  </span>
                )}
              </div>
            </div>
    ...
    <div className="animate-in fade-in slide-in-from-right-8 pb-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400">
              Select {title.includes("Sonic Aura") ? "1 song" : "10 songs"} (
              {selectedIds.size}/{title.includes("Sonic Aura") ? "1" : "10"})
            </p>
          </div>
        </div>
        <button
          ...
          className="px-6 py-2 bg-[#1DB954] text-black font-bold rounded-full disabled:opacity-30 transition-all"
        >
          Generate
        </button>
      </div>
      ...
      {isUnderground && (
        <div className="mb-8 p-4 bg-zinc-900 rounded-xl border border-zinc-800">
          <div className="flex items-center gap-2 mb-3 text-sm text-zinc-400">
            ...
            <span>Uniqueness Score (New Green Palette)</span>
          </div>
          <div className="flex gap-2 h-6 rounded-lg overflow-hidden bg-zinc-800">
            <div className="w-1/3 bg-emerald-600 flex items-center justify-center text-[10px] text-white font-bold">
              Royal (&gt;80%)
            </div>
            <div className="w-1/3 bg-green-500 flex items-center justify-center text-[10px] text-black font-bold">
              Common (&gt;50%)
            </div>
            <div className="w-1/3 bg-lime-200 flex items-center justify-center text-[10px] text-black font-bold">
              Light (&lt;50%)
            </div>
          </div>
        </div>
      ...
      {isTimeline ? (
        <div className="space-y-8">
          <div className="p-6 rounded-3xl bg-gradient-to-b from-blue-400/20 to-zinc-900/50 border border-blue-300/20">
            <h3 className="text-xl font-bold text-blue-200 mb-4">
              Phase 1: The Foundation (Jan - Apr)
            </h3>
            {renderTrackList(candidates.filter((t: any) => t.phase === 1))}
          </div>
          <div className="p-6 rounded-3xl bg-gradient-to-b from-blue-600/20 to-zinc-900/50 border border-blue-500/20">
            <h3 className="text-xl font-bold text-blue-400 mb-4">
              Phase 2: The Evolution (May - Aug)
            </h3>
            {renderTrackList(candidates.filter((t: any) => t.phase === 2))}
          </div>
          <div className="p-6 rounded-3xl bg-gradient-to-b from-blue-900/20 to-zinc-900/50 border border-blue-800/20">
            <h3 className="text-xl font-bold text-blue-600 mb-4">
              Phase 3: The Now (Sep - Dec)
            </h3>
            {renderTrackList(candidates.filter((t: any) => t.phase === 3))}
          </div>
        </div>
      ) : (
    ...
    <div className="animate-in fade-in">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-green-500 text-black font-bold rounded-full hover:bg-green-400 transition-colors"
        >
      ...
      <div
        ref={resultsRef}
        className="p-8 bg-zinc-950 border border-zinc-800 rounded-3xl"
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {tracks.map((track: any, idx: number) => (
            <div
              key={track.id}
              className="group bg-zinc-900 rounded-xl p-4 relative border border-zinc-800"
            >
